...
if __name__ == '__main__':
    ...
    parser.add_argument("--model_type", help="model_type", type=str, default='Unet')
    ...
    sub_name = f'Model_{args.model_type}_{args.encoder}_bs_{args.bs}_{str(datetime.datetime.now().date())}'
    ...
    loaders = prepare_loaders(path=args.path, bs=args.bs,
                              ...
                              image_size=(args.height, args.width), augmentation=args.augmentation)
    test_loader = loaders['test']
    del loaders['test']

    model = get_model(model_type=args.model_type, encoder=args.encoder, encoder_weights=args.encoder_weights, activation=None)

    optimizer = get_optimizer(optimizer=args.optimizer, lookahead=False, model=model,
    ...
    if args.make_prediction:
        ...
        utils.unpack_checkpoint(checkpoint, model=model)
        runner = SupervisedRunner(model=model)
        class_params = {0: (0.45, 25000), 1: (0.65, 15000), 2: (0.5, 15000), 3: (0.55, 11000)}
